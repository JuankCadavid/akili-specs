...
- **Premise Ledger.** `design.md` carries one row for every claim the design takes as *given* about the existing system rather than creates, and those rows are written before the decisions that lean on them. A claim earns a row only when its falsity would change a design decision, a task, or the scope — true trivia stays out. A claim nothing in the repository settles keeps its row, marked `UNVERIFIED`, naming the check and the one owner who will settle it; a design that names no existing code, data, contract, or rule says so in a single stated-empty line rather than dropping the section. Row shape, the closed set of seven classes (`location`, `existence`, `data-env`, `other`, `live-path`, `shared-state`, `consumer`), the blast-radius triggers, and the citation rules are defined once in the command's Step 2.2 *Premise Ledger* block, which every other surface cites by name.
...
- **Review intensity.** Each task also carries a `Review` field (`skip-eligible`, `checklist`, `full`, `lenses`) plus a one-line reason. `skip-eligible` names a claim the task must earn at execute time (`/akili-execute` Step 2.3's *Review intensity* check) — never an instruction to skip outright, and a claim that turns out unearned still gets a normal conformance review. Step 3.3's presentation names every `skip-eligible` task with its reason before the user approves — not only inside the document — and the Verification Checklist confirms it happened. A `tasks.md` written before this field existed carries none: every one of its tasks gets a conformance Reviewer, unchanged (FR-10). When earned, the task closes with a `REVIEW_SKIPPED` record instead of a Reviewer `PASS`.
...
- The user approves requirements, design, and tasks before implementation begins. The design summary prints the Premise Ledger's count line as written and every open `UNVERIFIED` row in full, so a row the user could correct never stays buried in the document; an open row of `High` impact makes **Review Design** the recommendation, and **Continue** stays available.
