...
- **Falsifiability:** a task's verification must name an input that would make the check fail, not only a criterion for passing. Six rules apply:
  - *Expressible falsifier* — name the mutation and the fixture row where correct and mutated readings diverge.
  ...
  - *Rendered-measurement checklist* — only when a gate asserts size, overflow, visibility, position, or containment: baseline, production fonts, real geometry (never class presence), at least two viewports including the squeeze band, effective CSS px with zoom named, and clip containment.
