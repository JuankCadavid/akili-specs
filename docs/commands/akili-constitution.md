...
The constitution summary written into the root `CLAUDE.md`/`AGENTS.md` carries three rules that bind every command:
...
- **The optional `Integration Branch: <name>` pin** — written immediately after `Default Branch: <name>` for projects that route spec branches through one shared integration branch before it reaches the default branch. Detection is **a question to the user, never inferred from git or branch-name heuristics**: set it when spec branches merge into the named branch and that branch reaches the default branch on a release cadence; leave it unset for a trunk-based flow, saying so in one line. Confirmation shows the exact assertion the pin makes — this branch is the team's single integration point, applies run here serially, and its merges to the default branch carry applied standardizations forward — and the pin takes exactly one name, distinct from the default branch. When it exists, the `kaizen` skill's **apply-capable predicate** resolves to this branch alone, and the default branch records pending items instead of applying, the same non-writing behavior a spec branch already has. **Safe Update** adds the pin only when it is absent and never rewrites an existing one; when it does add the pin it also rewrites the adjacent write-discipline sentence below to apply-capable wording and notes that already-deployed `.agents/*` personas keep the old phrasing until re-scaffolded (the next `/akili-audit` reports that as persona drift).
- **The shared-file write discipline** — on a spec branch, lifecycle side-effect writes (kaizen standardizations, `/akili-archive` guide and TRD syncs, `/akili-audit` outputs) never edit shared guides, `.agents/` personas, packaged templates, or the TRD. Each would-be edit is recorded as a pending item and applied on the apply-capable branch — the default branch, or the pinned integration branch when one exists, never both. The packaged `leader.md` and `implementer.md` personas carry the same guardrail, with its exemption for files an approved `tasks.md` names as the spec's own deliverable — those are the spec's product, protected by the normal review flow, not a side effect. An inline-drafted persona must carry the guardrail *and* the exemption; dropping the exemption is not a shorter rule but a wrong one.
