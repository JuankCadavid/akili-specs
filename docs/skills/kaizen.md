...
- **Measure** hunts MUDA (waste) in the spec's own evidence: Reviewer rework attempts, pivots, PRODUCT_BUGs, severe judgment-day findings, validation WARN/FAIL, quick escalations, drift (the most recent report under `docs/specs/audits/`, legacy `docs/specs/drift-report.md` as fallback), tasks closed under `REVIEW_WAIVED` (by flag), tasks closed under `REVIEW_SKIPPED` (by task, with its predicate evidence), and escaped defects — a defect found after a task closed under a `REVIEW_SKIPPED` record. **A run is never classified clean while a `REVIEW_SKIPPED` task carries an escaped defect against it** — that is precisely the run whose retrospective must run.
