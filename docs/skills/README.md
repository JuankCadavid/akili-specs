...
| [`kaizen`](kaizen.md) | core | Juan Carlos Cadavid — jcadavid.com (MIT) | Bounded two-phase continuous-improvement loop: the retrospective (Measure → Learn → Standardize → Record) on any branch, then Apply Mode over the pending backlog on the default branch | `/akili-archive`; **Apply Mode** on request ("apply pending kaizen standardizations") or offered by `/akili-archive` on the default branch; its Active Lessons digest is read by propose/specify/execute/resume |
