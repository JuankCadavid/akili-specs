...
**Mid-climb events** *(added by the T6 Pivot, 2026-09-19)*: a later runtime event in the same attempt, of any kind, **continues the climb from the rung already reached** — or from the new event's own entry rung when that rung is higher *(clarified by the Leader at the T2 owed clause, 2026-09-19: a provider-limit death whose worker context survives still goes to rung 3 — the scenario above already forbids spawning a fresh worker while the old one can answer, and rung 2 is a fresh spawn)* — a spent rung is never re-run, and a rung whose condition does not hold (rung 3 when no worker context exists) is skipped — and the attempt's `runtime events:` line names every event and the one rung that recovered the attempt. The ladder therefore stays bounded per attempt: a flapping host cannot loop rungs 1–2. The command SHALL state each event kind's entry rung (the paragraph above) and this rule in the *Runtime-failure fallback* block.
