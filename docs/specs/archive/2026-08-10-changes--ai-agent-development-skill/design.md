...
| `framework-selection.md` | Dated matrix; win/lose conditions per row; recognition rows for CrewAI (role-based crews; wins on rapid role-play prototypes, loses on graph-level control of every transition or ecosystem standardization — **not** durability: CrewAI Flows persist state across restarts per its docs; corrected during T2, see execution.md Spec Correction) and AutoGen (row records succession by the Microsoft Agent Framework) | Operational guidance for recognition rows (FR-2 BUT) |
