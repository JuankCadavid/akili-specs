...
| KZ-changes--model-routing-cost-rebaseline-1 | A figure read off a screenshot is proposal context, not a source — open the page (or run the command), confirm column and unit, and pin it before it enters a requirement or design cell | changes/model-routing-cost-rebaseline | **High** | Methodology | `/akili-specify` Step 1.2 (numbers-from-images bullet) | **Applied** |
| KZ-changes--model-routing-cost-rebaseline-2 | A Leader brief dispatched before a Pivot must be re-issued with the amended text before the task resumes — a pre-pivot brief carries the superseded value into a FAIL | changes/model-routing-cost-rebaseline | Medium | Methodology | `/akili-execute` Pivot Protocol step 4 | **Applied** |
| KZ-changes--branch-safe-kaizen-1 | Verify the worker's artifact/working tree before the single poke of the idle-without-report protocol — a poke citing stale evidence invites a double-apply | changes/branch-safe-kaizen | Low | Methodology | `.claude/templates/leader.md` (idle protocol reordered) | **Applied** |
| KZ-changes--branch-safe-kaizen-2 | A Correction Closure forward sweep greps the superseded concept's paraphrase terms, not only its literal strings | changes/branch-safe-kaizen | Medium | Methodology | `/akili-specify` Correction Closure item 1 | **Applied** |
