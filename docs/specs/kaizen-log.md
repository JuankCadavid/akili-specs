...
| KZ-changes--agents-md-canonical-1 | A removal or rename sweep is keyed on the **obligation**, not on the identifier being removed — a rule that mandates the thing you are deleting may never name it, and a heading-level inventory cannot see a bullet lost under a surviving heading | changes/agents-md-canonical | Medium | Methodology | `/akili-specify` Step 3.2 (task-quality rules) | **Applied** |
