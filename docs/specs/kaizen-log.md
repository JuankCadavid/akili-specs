...
|---|---|---|---|---|---|---|
| KZ-changes--gate-falsifiability-1 | A closure walkthrough over the cases its rules cite is an inert fixture — name at least one held-out case and judge the general sentence with its parenthetical example stripped | changes/gate-falsifiability | **High** | Methodology | `/akili-specify` Step 3.2 (walkthrough bullet after the presence-assertion rule) | **Applied** |
| KZ-changes--gate-falsifiability-2 | Requirement content is dropped in shipping and passes a Reviewer that checks scenarios — walk each FR statement, table and every paragraph under it term by term; a missing obligation is never an ADVISORY — **recurred (changes/leader-brief-contract: FR-4's terminal-branches paragraph never shipped, PASSed "term for term", found at the closure gate, cost a Pivot)** | changes/gate-falsifiability · changes/leader-brief-contract | **High** | Methodology | `.claude/templates/reviewer.md` Audit Checklist (Requirement Conformance) | **Applied** |
| KZ-changes--leader-brief-contract-1 | A rule inserted into an existing paragraph gets contradicted by its surviving neighbour — read every paragraph that received an insertion whole before review; a warning in the brief did not prevent the repeat | changes/leader-brief-contract | Medium | Methodology | `.claude/templates/leader.md` item 4 (pre-review restatement sweep, extended) | **Applied** |
| KZ-changes--leader-brief-contract-2 | A `Falsifier`, verification count or expected outcome that states a present-tense reading is run or walked before it is written — three unexecuted readings cost two `tasks.md` amendments and pulled a closure walk to a false PASS | changes/leader-brief-contract | Medium | Methodology | `/akili-specify` Step 3.2 Falsifiability rule 1 | **Applied** |
| KZ-changes--kaizen-loop-closure-1 | When a design adds a value to an enumerated type (status, kind, mode, branch context), the surface table walks **every existing consumer step** against the new value — naming only the steps you add left three sentences readable two ways and cost a Pivot | changes/kaizen-loop-closure | **High** | Methodology | `/akili-specify` Step 2.2 guidelines (new enumerated values walk their consumers) | **Applied** |
| KZ-changes--kaizen-loop-closure-2 | Before spawning the Reviewer on a rules-document edit, grep the file for the superseded phrasing and its paraphrases — a surviving in-file restatement is brief non-conformance, not a Reviewer round | changes/kaizen-loop-closure | Medium | Methodology | `.claude/templates/leader.md` item 4 (pre-review restatement sweep) | **Applied** |
...
| KZ-001 | When pinning a source, read it past the section you came for — the costliest review FAIL class is a claim contradicted elsewhere in its own pinned source — **recurred (3 first-attempt pin/quotation FAILs in changes/codex-install-target: T4 tenant negative, T5 flagship overstatement, reopen-wave fused `/goal` quotes)** | changes/ai-agent-development-skill · changes/codex-install-target | **High** | Methodology | `.claude/templates/implementer.md` (Verification Rigor — "a quotation is a claim of byte identity") | **Applied** |
