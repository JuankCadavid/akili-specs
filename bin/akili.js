...
function writeUpdateCache(latest) {
  try {
    const tmpPath = `${UPDATE_CACHE_PATH}.${crypto.randomBytes(6).toString("hex")}.tmp`;
    fs.writeFileSync(tmpPath, JSON.stringify({ checkedAt: Date.now(), latest }));
    fs.renameSync(tmpPath, UPDATE_CACHE_PATH);
  } catch {
...
function runNotifications(args, action) {
  ...
  if (action === "enable") {
    ...
    const tmpPath = `${settingsPath}.${crypto.randomBytes(6).toString("hex")}.tmp`;
