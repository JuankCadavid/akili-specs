...
- **The optional `Integration Branch: <name>` pin:** offer a second literal line, written immediately after `Default Branch: <name>` in the same summary block, for projects that route spec branches through one shared integration branch before it reaches the default branch.
  - **Detection is a question to the user, never inferred:** ask directly whether spec branches merge into one branch first, and that branch reaches the default branch on a release cadence. When the answer is yes, ask for the branch name as its own explicit step — a yes/no answer is not a name, and the pin is written with that name only. No heuristic resolves the `Integration Branch:` pin from a branch-name list or merge history — only the user knows which branch integrates.
  - **When to set the `Integration Branch:` pin:** spec branches merge into the named branch, and that branch reaches the default branch on release cadence.
  - **When not to set it:** a trunk-based flow, where spec branches merge directly to the default branch — say so in one line and write no `Integration Branch:` pin.
  - **Confirm before writing the `Integration Branch:` pin:** show the user the exact assertion it makes, then write only on confirmation — *this branch is the team's single integration point; apply passes run here serially, and its merges to the default branch carry applied standardizations forward.*
  - **Exactly one name** for the `Integration Branch:` pin, and it must differ from the `Default Branch:` name.
  - **What the `Integration Branch:` pin changes:** the `kaizen` skill's apply-capable predicate (Branch Context) now resolves to the pinned integration branch as the sole apply-capable branch, and the default branch records pending items instead of applying — the same non-writing behavior a spec branch already has.
  - **Safe Update mode** adds the `Integration Branch:` pin only when it is absent from the existing summary; it never rewrites an existing pin. When Safe Update does add the pin, it also rewrites the adjacent write-discipline sentence below — which previously named the default branch as the only place edits land — to apply-capable wording, and states in one line that already-deployed `.agents/*` personas keep the old phrasing until re-scaffolded — the next `/akili-audit` reports that as persona drift (the predecessor's known W-1 interaction)
- **The shared-file write discipline:** on a spec branch, lifecycle side-effect writes — kaizen standardizations, `/akili-archive` guide/TRD syncs, `/akili-audit` outputs — **never edit shared guides, `.agents/` personas, packaged templates, or the TRD**. Each would-be edit is recorded as a pending item and applied on the apply-capable branch (the default branch, or the pinned integration branch when one exists — never both). This belongs in the root guides because it binds every command and every agent, including sessions that never load the `kaizen` skill
