...
- **Wind Down Before You Run Out:** a rework loop is up to 3 attempts × (Implementer + Reviewer) — six delegated round trips plus adjudication. Do not open one you cannot see through. When context is running low, finish or park the task in flight, stop starting new ones, and spend what remains on `execution.md`: the audit trail **is** the handoff, and `/akili-resume` can only rebuild what was written. Park explicitly (`[~]` plus the full attempt history), never silently. And never leave a **supervised** delegation outstanding — a worker told to report back to a coordinator who is gone produces work that may be complete and correct with nothing to record it; transfer ownership instead, or park the task and let the next session re-spawn cleanly. **That is the default, not a ban the user cannot lift** — when they explicitly ask you to supervise ("wait for the result", "wire the response back"), supervise, say in one line that context is tight, and **budget for the landing**: blocking on a completion message costs wall-clock, but *receiving* it costs context, so cap the payload you read in and pull detail from the worker's report file only if forced. See *Winding down* in `.agents/leader.md`.
