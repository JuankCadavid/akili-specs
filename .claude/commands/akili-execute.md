...
- **Codex:** if the project has `.codex/agents/akili-implementer.toml` / `akili-reviewer.toml` (scaffolded by `/akili-constitution` Step 8E), request the named `akili-implementer` / `akili-reviewer` role by name with the brief — Codex spawns it, routes the work, and waits; consume the consolidated result it returns. Otherwise, fall back to a sub-prompt seeded with the persona file, as today. **Unverified:** the docs describe this as model-driven orchestration, not a named tool contract — do not claim a `spawn_agent`-style call exists; FR-10 live validation confirms the mechanism.
...
**Codex:** no verified equivalent, so this guidance makes no claim for Codex (the same wording `docs/flow.md` uses for OpenCode). Do not claim `/goal`-style unattended looping for Codex until FR-10 live validation exercises it; until then, run Codex sessions attended.
