...
- **A removal or rename sweep is keyed on the obligation, not on the identifier being removed.** A rule that mandates the thing you are deleting may never name it — *"written to **both** root guides"* survives every `grep` for the filename, and a heading-level inventory cannot see a bullet lost under a heading that survives in both files. Specify the sweep on what the rule *obliges* (the collective plural, `both`, `each`) and at the granularity the loss would occur, then read every hit.
